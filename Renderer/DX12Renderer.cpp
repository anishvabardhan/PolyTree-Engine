...
{
	int m_meshletVisualizer = 0;
};

static int const k_meshletDebugConstantSlot = 2;
static int const k_directionalLightConstantSlot = 3;
...
void DX12Renderer::StartUp()
{
	...
	m_meshletDebugCBO = CreateConstantBuffer(sizeof(MeshletDebugConstants), std::wstring(L"Meshlet Debug"));
...
void DX12Renderer::CreateShaderResourceView(ID3D12Resource* defaultBuffer, ID3D12DescriptorHeap*& srv, DXGI_FORMAT srvFormat, D3D12_SRV_DIMENSION srvDims, UINT bufferSize, UINT numElements)
{
	D3D12_DESCRIPTOR_HEAP_DESC srvDescHeap = {};
	srvDescHeap.Type = D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV;
	srvDescHeap.NumDescriptors = 1;
	...
	HRESULT hr = m_device->CreateDescriptorHeap(&srvDescHeap, IID_PPV_ARGS(&srv));
	...
	m_device->CreateShaderResourceView(defaultBuffer, &srvDesc, srv->GetCPUDescriptorHandleForHeapStart());
}
...
void DX12Renderer::CreateDepthStencilBuffer()
{
	...
	defaultResourceDesc.DepthOrArraySize = 1;
	defaultResourceDesc.MipLevels = 1;
	defaultResourceDesc.Format = DXGI_FORMAT_D32_FLOAT;
	defaultResourceDesc.SampleDesc = {1, 0};
	defaultResourceDesc.Layout = D3D12_TEXTURE_LAYOUT_UNKNOWN;
...
void DX12Renderer::BindMeshBuffer(Model* model, Texture* texture)
{
	...
	m_commandList->SetGraphicsRootDescriptorTable(6, model->m_modelDescHeap->GetGPUDescriptorHandleForHeapStart());
}
...
void DX12Renderer::CreateMeshShadingPipelineState()
{
	...
	m_meshPipelineStateDescriptor->RasterizerState.CullMode = D3D12_CULL_MODE_BACK;
	...
	m_meshPipelineStateDescriptor->DSVFormat = DXGI_FORMAT_D32_FLOAT;
...
void DX12Renderer::SetModelConstants(RootSig pipelineMode, Mat44 const& modelMatrix, Rgba8 const& modelColor, ConstantBuffer* modelCBO)
{
	...
	if (modelCBO == nullptr)
	{
		...
		BindConstantBuffer(k_modelConstantSlot, m_modelCBO, pipelineMode);
	}
	else
	{
		CopyCPUToGPU(&modelConstants, modelCBO->m_size, modelCBO);
		BindConstantBuffer(k_modelConstantSlot, modelCBO, pipelineMode);
	}
}
...
void DX12Renderer::SetDirectionalLightConstants(Vec3 sunDirection, float sunIntensity, float ambientIntensity)
{
	...
	BindConstantBuffer(k_directionalLightConstantSlot, m_directionalLightCBO, RootSig::MESH_SHADER_PIPELINE);
}
...
void DX12Renderer::SetMeshletDebugConstants(MeshletDebugConstants debugConstants)
{
...
void DX12Renderer::CreateRootSignature(RootSig rootSig)
{
	...
	case DEFAULT_PIPELINE:
	{
		...
		hr = m_device->CreateRootSignature(0, pSerializedRS->GetBufferPointer(), pSerializedRS->GetBufferSize(), IID_PPV_ARGS(&m_rootSig[rootSig]));
	...
	{
		D3D12_DESCRIPTOR_RANGE  structuredBufferDescriptorTableRanges[5];
		...
		structuredBufferDescriptorTableRanges[0].BaseShaderRegister = 1;
		...
		structuredBufferDescriptorTableRanges[1].BaseShaderRegister = 2;
		...
		structuredBufferDescriptorTableRanges[2].BaseShaderRegister = 3;
		...
		structuredBufferDescriptorTableRanges[3].BaseShaderRegister = 4;
		...
		structuredBufferDescriptorTableRanges[4].BaseShaderRegister = 5;
		...
		rootParam[0].Descriptor.ShaderRegister = 0;
		rootParam[0].Descriptor.RegisterSpace = 0;
		rootParam[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_MESH;
		...
		rootParam[2].ShaderVisibility = D3D12_SHADER_VISIBILITY_MESH;
		...
		rootParam[6].DescriptorTable = descriptorTable;
...
{
	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE);

	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE);

	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE);

	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE);

	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE);

	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE);

	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE);

	CreatePipelineState(PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE);
	CreatePipelineState(PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE);
}
...
void DX12Renderer::CreatePipelineState(PipelineState pipelineState)
{
	...
	{
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE: //1
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE: //2
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE: //3
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE: //4
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE: //5
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE: //6
	{		
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE: //7
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE: //8
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE: //9
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE: //10
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE: //11
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE: //12
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_LESS_EQUAL, D3D12_DEPTH_WRITE_MASK_ALL);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE: //13
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE: //14
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE: //15
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE: //16
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE: //17
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE: //18
	{		
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_BACK, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE: //19
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE: //20
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE: //21
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_SOLID, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE: //22
	{
		SetPipelineStateDesc(D3D12_BLEND_SRC_ALPHA, D3D12_BLEND_INV_SRC_ALPHA, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE: //23
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ONE, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE] = pso;

		break;
	}
	case PipelineState::BLEND_OP_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE: //24
	{
		SetPipelineStateDesc(D3D12_BLEND_ONE, D3D12_BLEND_ZERO, D3D12_FILL_MODE_WIREFRAME, D3D12_CULL_MODE_NONE, D3D12_COMPARISON_FUNC_ALWAYS, D3D12_DEPTH_WRITE_MASK_ZERO);
		...
		m_PSO[BLEND_OP_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE] = pso;
...
void DX12Renderer::SetPipelineStateDesc(D3D12_BLEND srcBlend, D3D12_BLEND destBlend, D3D12_FILL_MODE filleMode, D3D12_CULL_MODE cullMode, D3D12_COMPARISON_FUNC depthCompFunc, D3D12_DEPTH_WRITE_MASK depthWriteMask)
{
	...
	m_pipelineStateDescriptor->DSVFormat = DXGI_FORMAT_D32_FLOAT;
...
{
	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::ENABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCB_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_BACK && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_OP_SAMPLE_POINT_RAST_WCB_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_SCN_DEPTH_ENABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::SOLID_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_OP_SAMPLE_POINT_RAST_SCN_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ALPHA && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AL_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::ADDITIVE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE]);
	}

	if (m_desiredBlendMode == BlendMode::OPAQUE && m_desiredDepthStencilMode == DepthMode::DISABLED && m_desiredRasterizerMode == RasterizerMode::WIREFRAME_CULL_NONE && m_desiredSamplerMode == SamplerMode::POINT_CLAMP)
	{
		m_commandList->SetPipelineState(m_PSO[BLEND_AD_SAMPLE_POINT_RAST_WCN_DEPTH_DISABLE]);
	}
}
...
void DX12Renderer::BeginCamera(Camera& camera, RootSig pipelineMode)
{
	...
	BindConstantBuffer(k_cameraConstantSlot, camera.m_cameraCBO, pipelineMode);
...
void DX12Renderer::DrawMesh(int numOfMeshlets)
{
